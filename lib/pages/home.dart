...
class _userHomeState extends State<userHome> {
  ...
  final List<DropdownItem> dropdownItems = [
    DropdownItem(text: 'Following', icon: Icons.people, color: Colors.black),
    DropdownItem(text: 'Favourites', icon: Icons.favorite, color: Colors.black),
  ];
  void toggleDropdown() {
    setState(() {
  ...
  child: ListView.builder(
    ...
    itemBuilder: (context, index) {
      // Get the user's latest post by accessing the last element in the posts list
      Post latestPost = usersInfo[index].posts!.last;
      ...
      return UserPosts(
        ...
        if (isDropdownOpen)
          Positioned(
            top: 90, // Adjust position as needed to fit under appbar title
            left: 10,
            child: GestureDetector(
              onTap: () {}, // Stops onTap from propagating to close dropdown
              child: Container(
                width: 145,
